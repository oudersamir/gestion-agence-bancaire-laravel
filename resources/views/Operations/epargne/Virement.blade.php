...
error1('erreur');
      ...
      @endif
            <h2> Virement de Compte Epargne</h2>

   <form   action="{{url('Virement_Compte_Epargne')}}" method="post">
...
Options</h4>
 ...
 <a href="{{url('Effectuer_Virement_epargne')}}" class="list-group-item bg-success list-group-item-action active"><b>Effectuer un Virement</b></a>
  <a href="{{url('Effectuer_Retrait_Epargne')}}" class="list-group-item  list-group-item-action "><b>Effectuer un Retrait</b></a>
  <a href="{{url('Effectuer_Viresement_Epargne')}}" class="list-group-item list-group-item-action"><b>Effectuer un Versement</b></a>
 ...
 $('.dynamic').change(function(){
  ...
  {var client= 'c';
   ...
   $.ajax({
    url:"{{ route('OperationController.fetch_epargne') }}",
    method:"POST",
    data:{select:select, value:value, _token:_token, dependent:dependent,value2:value2,client:client},
...
var id_client2 =$('#client_id_client').val();
var id_client2_text =$('#client_id_client').find("option:selected").text();
  $('#client_id_client1').empty();
  $( "#client_id_client1" ).append( ' <option selected>Sélectionner le compte client</option>');

$( "#client_id_client1" ).append( '<option value="'+id_client2+'">'+id_client2_text+'</option>');
     //  $('#id_comptes').val('');
...
var str1= $('#id_comptes1').find("option:selected").text();
  if(str1.indexOf("compte courant") != -1){
//alert(str1+"  rah kayn ");
$('#type_virement').val('epargne_courant');
} else {
//
//alert("  rah  makaynch ");
$('#type_virement').val('epargne_epargne');
